...
pub fn main() !void {
    ...
    while (it.next()) |line| {
        ...
        var i: u7 = 0;
        var incorrect = false;
        line_loop: while (i < line_list.items.len) {
            const num = line_list.items[@intCast(i)];
            used |= bit_128 << @truncate(num);
            // std.debug.print("line: {d} num: {d}\n", .{ line_count, num });
            for (line_list.items, 0..) |i_num, item_pos| {
                const i_7: u7 = @truncate(i_num);
                if (map[@intCast(num)] >> i_7 & bit_128 == bit_128) {
                    if ((used >> i_7) & bit_128 == bit_0_128) {
                        std.debug.print("line: {d} not found: {d} for {d}. line count: {d} \n", .{ line_count, i_num, num, line_list.items.len });
                        incorrect = true;
                        const removed = line_list.orderedRemove(item_pos);
                        try line_list.insert(@intCast(i), removed);
                        i = 0;
                        used = 0;
                        continue :line_loop;
                    }
